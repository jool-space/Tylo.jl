...
# The mask may be `nothing`.
@inline baseline_valid(mask,j,row)=mask===nothing || (@inbounds mask[j,row])
...
function baseline_softmax_kernel!(out,input,mask)
    ...
    for j in lane+Int32(1):Int32(32):width
        if row<=rows && baseline_valid(mask,j,row)
    ...
    for j in lane+Int32(1):Int32(32):width
        if row<=rows
            @inbounds out[j,row]=baseline_valid(mask,j,row) && s>0f0 ? exp(Float32(input[j,row])-m)/s : 0f0
        end
    end
    nothing
end
